...
1. `global.image` must be set to `consul:1.8.0-beta2` because Kubernetes support
...
1. Note `global.image: consul:1.8.0-beta2`. Kubernetes support for federation is currently only available
